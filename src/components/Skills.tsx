...
import { Code, Database, Server, Layers, Terminal } from "lucide-react";
import { useEffect, useRef } from "react";
...
const Skills = () => {
  ...
  const pos = useRef(0);
  const speed = useRef(0.35); // auto-scroll speed
  ...
  return (
    <section id="skills" className="py-24 relative overflow-hidden">
      ...
      <div className="container relative z-10 px-4">
        ...
        {/* ⭐ MARQUEE (NO SCROLLBAR, FULLY FUNCTIONAL) */}
        <div
        ...
        >
          <div
            ref={trackRef}
            className="flex gap-8 whitespace-nowrap cursor-grab active:cursor-grabbing select-none"
          ...
          >
            {[...skillCategories, ...skillCategories].map((cat, i) => (
              <div
                key={cat.title + i}
                className="glass rounded-2xl p-6 min-w-[320px] max-w-xs shadow-lg hover:border-primary/50 transition-all"
              >
                <div className="flex items-center gap-4 mb-6">
                  <div className={`p-3 rounded-xl bg-${cat.color}/10`}>
